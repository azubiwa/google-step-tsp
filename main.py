...
def crossed_city(cities, cities_path, n, m): # n, n+1番目の辺とm, m+1番目の辺の交差判定
    ...
    # n, mの辺の長さの2乗の合計
    # nm_dist = dist(n_x1, n_y1, n_x2, n_y2) + dist(m_x1, m_y1, m_x2, m_y2)
    ...
    # mixed_dist = dist(n_x1, n_y1, m_x1, m_y1) + dist(n_x2, n_y2, m_x2,)
    ...
    while maybe_crossing: # 交差する可能性のある頂点が空になるまで
        ...
        for i in range(len(cities)):
            if crossed_city(cities, cities_path, city_index, i):
                ...
                break
        if isCrossing:
            maybe_crossing.append(cities_path[city_index])
        maybe_crossing = list(set(maybe_crossing))
    return cities_path

def for_two_opt(cities, cities_path):
    isOK = False
    while not isOK:
        isOK = True
        for i in range(len(cities)):
            for j in range(i+2, len(cities)):
                if crossed_city(cities, cities_path, i, j):
                    cities_path[i+1:j+1] = reversed(cities_path[i+1:j+1])
                    isOK = False
        print(cities_path)
    return cities_path
...
def TSP_solve(cities):
    ...
    # cities_path = for_two_opt(cities, cities_path)
    return cities_path
